...
CREATE TABLE IF NOT EXISTS vendor_promos (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT vendor_promos_max_claims_check CHECK (max_claims IS NULL OR max_claims > 0),
  CONSTRAINT vendor_promos_claimed_count_check CHECK (claimed_count >= 0),
  CONSTRAINT vendor_promos_time_window_check CHECK (ends_at IS NULL OR starts_at IS NULL OR ends_at > starts_at)
);
...
CREATE TABLE IF NOT EXISTS customer_promo_claims (
  ...
  CONSTRAINT customer_promo_claims_status_check CHECK (status IN ('claimed', 'redeemed', 'expired', 'canceled')),
  CONSTRAINT customer_promo_claims_once_per_customer UNIQUE (promo_id, customer_user_id)
);
