...
interface Props {
  ...
  addMenuItem: (formData: FormData) => Promise<unknown>;
  deleteMenuItem: (formData: FormData) => Promise<unknown>;
}

function toMenuItem(value: unknown): VendorManageMenuItem | null {
  ...
  return {
    id: candidate.id,
    name: typeof candidate.name === "string" ? candidate.name : "Untitled item",
    description:
      typeof candidate.description === "string" ? candidate.description : null,
    price_cents:
      typeof candidate.price_cents === "number" ? candidate.price_cents : null,
    is_gluten_free:
      typeof candidate.is_gluten_free === "boolean"
        ? candidate.is_gluten_free
        : null,
    is_spicy:
      typeof candidate.is_spicy === "boolean" ? candidate.is_spicy : null,
    is_vegan:
      typeof candidate.is_vegan === "boolean" ? candidate.is_vegan : null,
    is_vegetarian:
      typeof candidate.is_vegetarian === "boolean"
        ? candidate.is_vegetarian
        : null,
  };
}
...
export function VendorManageMenuSection({
  ...
  const handleAddItem = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const createdItem = toMenuItem(result);

      if (createdItem) {
        setMenuItems((prev) => [...prev, createdItem]);
        formRef.current.reset();
        setFeedback(
          "Item added. You can add another or close this window when you're done.",
        );
      }
    } catch {
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[#757575]">
            Add a few of your signature dishes. These show on your public
            profile with dietary icons.
          </p>
          {menuItems.length > 0 && (
